...
async function processJob(job) {
  ...
  const [business, customers, appointments, payments, transactions, bills, invoices, accounts, categories, vendors, methods, media, attachments] = await Promise.all([
    prisma.business.findUniqueOrThrow({ where: { id: job.businessId }, include: { settings: true } }),
    prisma.customer.findMany({ where: { businessId: job.businessId, createdAt: { lte: job.toDate } } }),
    prisma.appointment.findMany({ where: { businessId: job.businessId, startAt: range }, include: { serviceLines: true, actualServiceLines: true } }),
    prisma.appointmentPayment.findMany({ where: { businessId: job.businessId, paidAt: range } }),
    prisma.financialTransaction.findMany({ where: { businessId: job.businessId, occurredAt: range } }),
    prisma.supplierBill.findMany({ where: { businessId: job.businessId, issueDate: range }, include: { lines: true } }),
    prisma.customerInvoice.findMany({ where: { businessId: job.businessId, issueDate: range } }),
    prisma.financialAccount.findMany({ where: { businessId: job.businessId } }), prisma.financialCategory.findMany({ where: { businessId: job.businessId } }), prisma.vendor.findMany({ where: { businessId: job.businessId } }), prisma.paymentMethod.findMany({ where: { businessId: job.businessId } }),
    prisma.mediaAsset.findMany({ where: { businessId: job.businessId, status: "READY", OR: [{ appointment: { startAt: range } }, { customer: { appointments: { some: { startAt: range } } } }] }, include: { variants: true } }),
    prisma.financialAttachment.findMany({ where: { businessId: job.businessId, OR: [{ transaction: { occurredAt: range } }, { bill: { issueDate: range } }] } }),
  ]);
  const files = [];
  ...
  const manifest = { schemaVersion: 1, generatedAt: new Date().toISOString(), period: { from: job.fromDate, to: job.toDate }, business: { id: business.id, name: business.name, slug: business.slug, settings: business.settings }, counts: Object.fromEntries(Object.entries(tables).map(([name, rows]) => [name, rows.length])), records: plain(tables) };
  files.push({ name: "manifest.json", data: JSON.stringify(manifest, null, 2) });
  for (const invoice of invoices) files.push({ name: `documents/invoices/${safeName(invoice.invoicePrefix)}-${String(invoice.invoiceNumber).padStart(5, "0")}.pdf`, data: pdf([business.name, `Invoice ${invoice.invoicePrefix}-${String(invoice.invoiceNumber).padStart(5, "0")}`, `Issued ${invoice.issueDate.toISOString().slice(0, 10)}`, `Amount ${invoice.currency} ${invoice.amount}`, invoice.dueDate ? `Due ${invoice.dueDate.toISOString().slice(0, 10)}` : "Due on receipt"]) });
  ...
  const archive = zip(files); const key = `${job.businessId}/exports/${job.id}.zip`; const checksum = createHash("sha256").update(archive).digest("hex");
