...
import { resolvePublicServiceCategory, type PublicLocale } from "@/lib/public-site";
...
export function PublicServices({ categories, locale, empty }: { categories: ServiceCategory[]; locale: PublicLocale; empty: string }) {
  ...
  return <div className="public-service-browser">
    <div aria-label={locale === "fa" ? "دسته‌بندی خدمات" : "Service categories"} className="public-service-tabs" role="tablist">
      {categories.map((category, index) => {
        ...
        return <button aria-controls="public-service-panel" aria-selected={selected} className="public-service-tab" data-active={selected ? "true" : undefined} key={category.id} onClick={() => setActiveId(category.id)} role="tab" type="button">
          ...
          <span className="min-w-0 flex-1"><strong className="block truncate" dir="auto">{category.name}</strong><small>{category._count.services} {locale === "fa" ? "خدمت" : "services"}</small></span>
    ...
    <section aria-live="polite" className="public-service-panel" id="public-service-panel" role="tabpanel">
      <div className="public-service-panel-heading"><span className="public-service-panel-icon"><CategoryIcon name={active.icon || "sparkles"} size={26}/></span><div className="min-w-0"><p>{countLabel}</p><h3 dir="auto">{active.name}</h3></div></div>
      <div className="public-service-list">{active.services.map((service, index) => <div className="public-service-item" dir="auto" key={service.id}><span dir="ltr">{String(index + 1).padStart(2, "0")}</span><Scissors size={15}/><strong>{service.name}</strong></div>)}</div>
    </section>
  </div>;
}
