...
import { clampPublicGalleryZoom, resolvePublicGalleryIndex, type PublicLocale } from "@/lib/public-site";
...
export function PublicGallery({ items, locale }: { items: GalleryItem[]; locale: PublicLocale }) {
  ...
  return <>
    <div className="public-work-grid">{items.map((item, index) => <button aria-label={`${locale === "fa" ? "باز کردن تصویر" : "Open studio image"} ${index + 1}`} className={`public-work-item ${index === 0 ? "public-work-featured" : ""}`} key={item.id} onClick={() => open(index)} type="button"><Image alt={item.alt} className="object-cover" fill sizes={index === 0 ? "(max-width:1024px) 100vw, 50vw" : "(max-width:768px) 50vw, 25vw"} src={item.src} unoptimized={item.unoptimized}/><span><Maximize2 size={17}/></span></button>)}</div>
