"use client";
/* eslint-disable @next/next/no-img-element -- image source is an authenticated media endpoint */
  ...
  return <>
    ...
    {open && createPortal(<div className="fixed inset-0 z-[999] flex items-start justify-center overflow-y-auto bg-[#02050a]/55 px-3 pt-[max(4rem,8vh)] backdrop-blur-xl" onMouseDown={(event) => { if (event.target === event.currentTarget) closeSearch(); }}>
      <section aria-label="Search Manisa" aria-modal="true" className="min-w-0 w-full max-w-2xl overflow-hidden rounded-3xl border border-white/15 bg-[#0b121d]/98 shadow-[0_35px_120px_rgba(0,0,0,.8)]" role="dialog">
        ...
        <div className="max-h-[min(65vh,38rem)] overflow-y-auto p-2">{query.trim().length < 2 ? <p className="px-4 py-10 text-center text-sm text-slate-500">Type at least 2 characters to search the studio.</p> : allResults.length ? allResults.map((result) => <Link className="flex items-center gap-3 rounded-2xl px-3 py-3 transition hover:bg-blue-500/10" href={result.href} key={`${result.type}:${result.id}`} onClick={closeSearch}><span className="flex size-10 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-blue-500/10 text-blue-300">{result.avatarId ? <img alt="" className="size-full object-cover" src={`/api/media/${result.avatarId}/avatar_small`}/> : <ResultIcon type={result.type}/>}</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold" dir="auto">{result.title}</span><span className="mt-0.5 block truncate text-xs text-slate-500" dir="auto">{result.type} · {result.subtitle}</span></span></Link>) : !loading && <p className="px-4 py-10 text-center text-sm text-slate-500">No matching items.</p>}</div>
      </section>
    </div>, document.body)}
  </>;
}
