...
export function AppointmentForm({ action, customers, services, appointment, initialCustomerId, initialStartAt }: AppointmentFormProps) {
  ...
  const steps = [{ label: "Customer" }, { label: "Services" }, { label: "Schedule" }, { label: "Details" }, { label: "Review" }];
    ...
    <form action={formAction} className="panel mx-auto max-w-5xl p-5 sm:p-7" onSubmit={(event) => {
      ...
      <WizardProgress current={step} steps={steps}/>
        ...
        <section className={step === 2 ? "block" : "hidden"}>
          <div className="mb-5 flex items-center gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-sky-300/10 text-sky-300"><CalendarDays size={19} /></span><h2 className="font-semibold text-white">Choose the date and time</h2></div>
        ...
        <section className={step === 3 ? "block" : "hidden"}>
          <div className="mb-5 flex items-center gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-amber-300/10 text-amber-200"><DollarSign size={19}/></span><h2 className="font-semibold text-white">Set the visit details</h2></div>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div><label className="label" htmlFor="expectedPrice">Estimated price</label><div className="relative"><DollarSign className="pointer-events-none absolute start-3.5 top-1/2 -translate-y-1/2 text-slate-500" size={17}/><input className="field h-12 ps-10" id="expectedPrice" inputMode="decimal" min="0" name="expectedPrice" onChange={(event) => setPrice(event.target.value)} placeholder="0.00" required step="0.01" type="number" value={price}/></div></div>
            <div className="sm:col-span-2"><label className="label" htmlFor="notes">Notes</label><textarea className="field min-h-24 resize-y" defaultValue={appointment?.notes || ""} id="notes" name="notes" placeholder="Optional notes"/></div>
          </div>
        </section>

      <section className={step === 4 ? "block" : "hidden"}><div className="overflow-hidden rounded-2xl border border-white/8"><div className="border-b border-white/8 px-5 py-4"><p className="text-sm font-semibold text-white">Appointment summary</p></div><div className="grid gap-5 p-5 sm:grid-cols-2">
