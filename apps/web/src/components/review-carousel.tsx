...
export function ReviewCarousel({ reviews, locale }: { reviews: Review[]; locale: PublicLocale }) {
  ...
  return <div aria-label={locale === "fa" ? "نظرهای مشتریان" : "Client reviews"} aria-roledescription="carousel" className="public-review-carousel min-w-0 overflow-hidden">
    <div className="flex transition-transform duration-700 ease-out" style={{ transform: `translateX(${(locale === "fa" ? 1 : -1) * active * 100}%)` }}>
      {reviews.map((review, index) => <article aria-hidden={index !== active} className="w-full shrink-0 px-0.5" key={review.id} lang={review.language}><div className="public-review-quote"><span aria-hidden="true" className="public-quote-mark">“</span><div className="relative flex gap-1 text-[var(--public-accent)]">{Array.from({ length: 5 }, (_, star) => <Star className={star < review.rating ? "fill-current" : "opacity-25"} key={star} size={17}/>)}</div><blockquote className="relative mt-6 text-lg leading-9 text-[var(--public-ink)] sm:text-xl" dir="auto">“{review.opinion}”</blockquote><p className="relative mt-6 text-sm font-bold" dir="auto">{review.reviewerName}</p></div></article>)}
    </div>
