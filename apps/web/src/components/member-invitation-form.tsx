...
export function MemberInvitationForm({ canInviteAdministrators = false }: { canInviteAdministrators?: boolean }) {
  ...
  return <form action={action} className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_10rem_auto]">
    ...
    <select className="input" name="role">{canInviteAdministrators && <><option value="ADMIN">Admin</option><option value="MANAGER">Manager</option></>}<option value="STAFF">Staff</option></select>
