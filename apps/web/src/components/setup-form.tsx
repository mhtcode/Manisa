...
import { setupStudio, type SetupState } from "@/server/actions/platform";
import { PasswordCreationFields } from "@/components/password-creation-fields";

export function SetupForm() {
  const [state, action, pending] = useActionState<SetupState, FormData>(setupStudio, {});
  return <form action={action} className="mt-7 space-y-4">
    ...
    <button className="button w-full" disabled={pending}>{pending ? "Creating studio…" : "Create studio owner"}</button>
  </form>;
}
