  ...
  return <form action={formAction} className="panel mx-auto max-w-4xl p-5 sm:p-7" onSubmit={(event) => {
    ...
    <section className={step === 2 ? "block" : "hidden"}>
      ...
      <div className="space-y-2">{payments.map((payment) => {
        const paidByOtherMethods = payments.reduce((sum, item) => item.key === payment.key ? sum : sum + Number(item.amount || 0), 0);
        const remaining = Math.max(0, totals.price - paidByOtherMethods);
        return <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,.7fr)_auto] gap-2" key={payment.key}><select className="field" name="paymentMethodId" onChange={(event) => setPayments((items) => items.map((item) => item.key === payment.key ? { ...item, methodId: event.target.value } : item))} value={payment.methodId}>{paymentMethods.map((method) => <option key={method.id} value={method.id}>{method.name}</option>)}</select><input aria-label="Payment amount" className="field" inputMode="decimal" max={remaining.toFixed(2)} min="0.01" name="paymentAmount" onChange={(event) => setPayments((items) => items.map((item) => item.key === payment.key ? { ...item, amount: event.target.value } : item))} placeholder={remaining.toFixed(2)} value={payment.amount}/><button aria-label="Remove payment" className="icon-button border-rose-400/20 text-rose-300" onClick={() => setPayments((items) => items.filter((item) => item.key !== payment.key))} title="Remove payment" type="button"><Trash2 size={15}/></button></div>;
      })}</div>
