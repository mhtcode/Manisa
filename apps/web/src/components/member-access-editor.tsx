...
import { businessPermissionKeys, rolePermissionDefaults, type BusinessPermission } from "@/lib/permissions";
    ...
    {open && <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/70 p-3 backdrop-blur-sm sm:items-center" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section aria-label={`${name} access`} aria-modal="true" className="panel max-h-[88vh] w-full max-w-2xl overflow-y-auto p-5" role="dialog">
        ...
        <form action={updateMembershipAccess.bind(null, id)} className="mt-5">
          <label className="label" htmlFor={`role-${id}`}>Role preset</label><select className="input" id={`role-${id}`} name="role" onChange={(event) => { const nextRole = event.target.value as Role; setSelectedRole(nextRole); setSelectedPermissions(rolePermissionDefaults(nextRole)); }} value={selectedRole}><option value="ADMIN">Admin · full studio access</option><option value="MANAGER">Manager · operations and finance</option><option value="STAFF">Staff · appointments and gallery</option></select>
