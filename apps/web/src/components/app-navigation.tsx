...
import { CalendarCheck2, CalendarDays, ChartNoAxesCombined, Images, Settings2, SwatchBook, UsersRound } from "lucide-react";
...
const primaryItems: MobileNavigationKey[] = ["report", "calendar", "gallery", "settings"];
...
export function DesktopNavigation({ locale }: { locale: AppLocale }) {
  ...
  return <nav aria-label="Main navigation" className="mt-6 space-y-1">
    {primaryItems.map((key) => {
      const [href, messageKey, Icon] = navigationItems[key];
      const active = isActive(pathname, href) || (key === "settings" && ["/appointments", "/customers", "/services"].some((item) => isActive(pathname, item)));
      return <Link aria-current={active ? "page" : undefined} className={`group flex items-center gap-3 rounded-xl border px-3 py-2.5 text-sm transition ${active ? "border-blue-400/25 bg-blue-500/10 font-medium text-blue-100" : "border-transparent text-slate-400 hover:bg-white/[0.05] hover:text-white"}`} href={href} key={href} prefetch={false}>
        <Icon className={active ? "text-blue-300" : "text-slate-500 group-hover:text-slate-300"} size={18}/>{t[messageKey]}
        {active && <span className="ms-auto size-1.5 rounded-full bg-blue-400 shadow-[0_0_12px_rgba(96,165,250,.75)]"/>}
      </Link>;
    })}
  </nav>;
}
