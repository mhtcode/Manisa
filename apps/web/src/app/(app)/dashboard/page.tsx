...
export default async function DashboardPage() {
  ...
  const cards = [
    ...
    ["Revenue this month", formatMoney(data.kpis.monthRevenue,currency,locale), DollarSign], ["Revenue this year", formatMoney(data.kpis.yearRevenue,currency,locale), WalletCards], ["Outstanding", formatMoney(data.kpis.outstanding,currency,locale), DollarSign], ["Hours this month", `${data.kpis.hours.toFixed(1)}h`, Clock3], ["Hourly income", formatMoney(data.kpis.hourlyIncome,currency,locale), TrendingUp],
  ] as const;
  return <><PageHeading title={locale === "fa" ? "داشبورد" : "Dashboard"} description={locale === "fa" ? `خوش آمدید، ${user.name}` : `Welcome back, ${user.name}. Here is how the business is doing.`} actions={<><Link className="button-secondary" href="/customers/new">+ Customer</Link><Link className="button-secondary" href="/services/new">+ Service</Link></>}/><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5">{cards.map(([label,value,Icon]) => <article className="stat" key={label}><div className="flex items-start justify-between"><p className="text-sm text-slate-500">{label}</p><Icon size={18} className="text-teal-300/70"/></div><p className="mt-4 text-2xl font-semibold tracking-[-0.03em]">{value}</p></article>)}</div><div className="mt-5"><AnalyticsCharts revenue={data.revenueByMonth} services={data.serviceRevenue}/></div><section className="panel mt-5"><div className="panel-header"><div><h2 className="font-medium">Upcoming appointments</h2><p className="mt-1 text-xs text-slate-500">Your next scheduled work</p></div><Link href="/appointments" className="text-sm text-teal-300">View all</Link></div>{data.upcoming.length ? <div className="divide-y divide-white/8">{data.upcoming.map((item) => <Link href={`/appointments/${item.id}`} key={item.id} className="flex flex-col gap-2 px-5 py-4 hover:bg-white/[0.02] sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><p className="font-medium">{customerName(item.customer)}</p><p className="mt-1 text-sm text-slate-500">{item.serviceNameSnapshot} · {formatBusinessDate(item.startAt,locale,timezone)}</p></div><StatusBadge status={item.status}/></Link>)}</div> : <div className="empty">No upcoming appointments.</div>}</section></>;
}
