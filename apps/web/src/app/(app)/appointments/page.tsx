...
import { BadgeCheck, CalendarClock, CalendarPlus, History, Layers3, TriangleAlert } from "lucide-react";
...
const stages = ["scheduled", "finalized", "historical", "exceptions", "all"] as const;
...
export default async function AppointmentsPage({ searchParams }: { searchParams: Promise<{ stage?: string; status?: string }> }) {
  ...
  const where: Prisma.AppointmentWhereInput = stage === "scheduled" ? { status: { in: ["SCHEDULED", "CONFIRMED"] } } : stage === "finalized" ? { status: "COMPLETED" } : stage === "historical" ? { status: "HISTORICAL" } : stage === "exceptions" ? { status: { in: ["CANCELLED", "NO_SHOW"] } } : {};
  const [appointments, scheduledCount, finalizedCount, historicalCount, exceptionCount, allCount] = await Promise.all([
  ...
  const stageTitle = stage === "scheduled" ? "Stage 1 · Scheduled estimates" : stage === "finalized" ? "Stage 2 · Finalized actuals" : stage === "historical" ? "Historical · Unreported" : stage === "exceptions" ? "Cancelled and no-show" : "All appointments";
  const stageDescription = stage === "scheduled" ? "Planned time and price—finalize each visit afterward to record actual results." : stage === "finalized" ? "Only these actual income and duration values feed business reporting." : stage === "historical" ? "Imported calendar history that never changes income or working-hour totals." : stage === "exceptions" ? "Appointments that did not proceed to finalization." : "Every appointment stage in one view.";
