...
import { BadgeCheck, CalendarCheck2, CircleDot, Clock3, LockKeyhole, Pencil, XCircle } from "lucide-react";
...
export default async function AppointmentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await prisma.appointment.findUnique({
    where: { id },
    include: { customer: true, serviceLines: { orderBy: { position: "asc" } }, actualServiceLines: { orderBy: { position: "asc" } } },
  });
  if (!item) notFound();
