...
export default async function TrashPage({ searchParams }: { searchParams: Promise<{ type?: string }> }) {
  ...
  const showFinancial = (selected === "all" || selected === "financial") && (hasBusinessPermission(user.role, user.permissionOverrides, "financial.view"));
  const [customers, appointments, photos, services, categories, paymentMethods, financialTransactions, bills, invoices, financialAccounts, financialCategories, vendors] = await Promise.all([
    ...
    selected === "all" || selected === "photo" ? prisma.mediaAsset.findMany({ where: { deletedAt: { not: null } }, include: { appointment: { include: { customer: true } } }, orderBy: { deletedAt: "desc" } }) : [],
    ...
    showFinancial ? prisma.financialTransaction.findMany({ where: { deletedAt: { not: null }, archivedAt: null }, orderBy: { deletedAt: "desc" } }) : [],
    showFinancial ? prisma.supplierBill.findMany({ where: { deletedAt: { not: null }, archivedAt: null }, orderBy: { deletedAt: "desc" } }) : [],
    showFinancial ? prisma.customerInvoice.findMany({ where: { deletedAt: { not: null }, archivedAt: null }, orderBy: { deletedAt: "desc" } }) : [],
    showFinancial ? prisma.financialAccount.findMany({ where: { deletedAt: { not: null }, archivedAt: null }, orderBy: { deletedAt: "desc" } }) : [],
    showFinancial ? prisma.financialCategory.findMany({ where: { deletedAt: { not: null }, archivedAt: null }, orderBy: { deletedAt: "desc" } }) : [],
    showFinancial ? prisma.vendor.findMany({ where: { deletedAt: { not: null }, archivedAt: null }, orderBy: { deletedAt: "desc" } }) : [],
  ]);
  const total = customers.length + appointments.length + photos.length + services.length + categories.length + paymentMethods.length + financialTransactions.length + bills.length + invoices.length + financialAccounts.length + financialCategories.length + vendors.length;
  ...
  return <>
    ...
    <BulkSelection action={bulkRestoreFromTrash} allIds={allIds} locale={user.settings?.locale || "en"} primary="restore" secondaryAction={bulkDeletePermanently}><section className="panel overflow-hidden">
      ...
      {categories.map((category) => <TrashRow deletedAt={category.deletedAt!} icon={Layers3} id={category.id} key={`category-${category.id}`} title={category.name} type="category">Service category · restoring or deleting also includes {category._count.services} related {category._count.services === 1 ? "service" : "services"}</TrashRow>)}
    ...
    {(financialTransactions.length > 0 || bills.length > 0 || invoices.length > 0 || financialAccounts.length > 0 || financialCategories.length > 0 || vendors.length > 0) && <section className="panel mt-5 overflow-hidden"><div className="panel-header"><h2 className="font-semibold">Financial records</h2></div><div className="divide-y divide-white/8">{financialTransactions.map((item) => <FinancialTrashRow action={restoreFinancialRecord.bind(null, "transaction", item.id)} deletedAt={item.deletedAt!} key={`transaction-${item.id}`} title={item.description}/>) }{bills.map((item) => <FinancialTrashRow action={restoreFinancialRecord.bind(null, "bill", item.id)} deletedAt={item.deletedAt!} key={`bill-${item.id}`} title={`${item.vendorNameSnapshot} · ${item.billNumber || "Bill"}`}/>)}{invoices.map((item) => <FinancialTrashRow action={restoreFinancialRecord.bind(null, "invoice", item.id)} deletedAt={item.deletedAt!} key={`invoice-${item.id}`} title={`${item.invoicePrefix}-${String(item.invoiceNumber).padStart(5, "0")}`}/>)}{financialAccounts.map((item) => <FinancialTrashRow action={restoreFinancialRecord.bind(null, "account", item.id)} deletedAt={item.deletedAt!} key={`account-${item.id}`} title={item.name}/>)}{financialCategories.map((item) => <FinancialTrashRow action={restoreFinancialRecord.bind(null, "category", item.id)} deletedAt={item.deletedAt!} key={`financial-category-${item.id}`} title={`${item.name} · ${item.type.toLowerCase()} category`}/>)}{vendors.map((item) => <FinancialTrashRow action={restoreFinancialRecord.bind(null, "vendor", item.id)} deletedAt={item.deletedAt!} key={`vendor-${item.id}`} title={item.name}/>)}</div></section>}
  </>;
}
