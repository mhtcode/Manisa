...
export default async function SettingsPage() {
  ...
  return <>
    ...
    <div className="space-y-5">
      {groups.map((group) => <section key={group.title}><h2 className="mb-2 px-2 text-xs font-semibold uppercase tracking-[.16em] text-slate-500">{group.title}</h2><div className="overflow-hidden rounded-2xl border border-white/10 bg-[#0d131d] shadow-[0_16px_40px_rgba(0,0,0,.16)]">{group.items.map(([href, label, description, Icon], index) => <Link className={`group flex items-center gap-3 px-4 py-3.5 transition hover:bg-blue-500/[0.07] active:bg-blue-500/[0.12] sm:px-5 ${index ? "border-t border-white/8" : ""}`} href={contextualDestinations.has(href) ? `${href}?from=settings` : href} key={href}><span className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-blue-400/12 bg-blue-500/8 text-blue-300"><Icon size={18}/></span><span className="min-w-0 flex-1"><span className="block text-sm font-semibold text-slate-100">{label}</span><span className="mt-0.5 block truncate text-xs text-slate-500">{description}</span></span><ChevronRight className="shrink-0 text-slate-600 transition group-hover:translate-x-0.5 group-hover:text-blue-300 rtl:rotate-180" size={18}/></Link>)}</div></section>)}
    </div>
