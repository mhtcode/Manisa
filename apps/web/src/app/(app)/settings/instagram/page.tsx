...
export default async function InstagramSettingsPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  const [connection, credential, env] = await Promise.all([
    ...
    prisma.instagramCredential.findUnique({ where: { id: "instagram" }, select: { appId: true, redirectUri: true } }),
    Promise.resolve(getServerEnv()),
  ]);
  const fallbackConfigured = Boolean(env.INSTAGRAM_APP_ID && env.INSTAGRAM_APP_SECRET && env.INSTAGRAM_REDIRECT_URI?.startsWith("https://"));
  ...
  const appId = credential?.appId || env.INSTAGRAM_APP_ID || "";
  ...
  return <>
    ...
    <div className="mb-5"><InstagramCredentialsForm appId={appId} encryptionReady={Boolean(env.INTEGRATION_ENCRYPTION_KEY)} redirectUri={redirectUri} source={source}/></div>
