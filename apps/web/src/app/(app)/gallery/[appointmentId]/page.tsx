...
import { ArrowLeft, CalendarDays, Download, Images, Scissors, Star, UserRound } from "lucide-react";
...
import { setAppointmentPhotoComparisonTag, setAppointmentPhotoFeatured } from "@/server/actions/appointments";
...
export default async function GalleryAlbumPage({ params, searchParams }: { params: Promise<{ appointmentId: string }>; searchParams: Promise<{ cursor?: string }> }) {
  ...
  const [appointment, allPhotoIds] = await Promise.all([prisma.appointment.findFirst({ where: { id: appointmentId, deletedAt: null, status: "COMPLETED" }, include: { customer: true, actualServiceLines: { orderBy: { position: "asc" } }, photos: { where: { deletedAt: null, ...(cursor ? { OR: [{ createdAt: { lt: cursor.createdAt } }, { createdAt: cursor.createdAt, id: { lt: cursor.id } }] } : {}) }, take: PAGE_SIZE + 1, orderBy: [{ createdAt: "desc" }, { id: "desc" }] } } }), prisma.mediaAsset.findMany({ where: { appointmentId, deletedAt: null }, select: { id: true } })]);
  ...
  return <>
    ...
    {canManage && photos.length > 0 && <SocialImageComposer photos={photos.map((photo) => ({ id: photo.id, comparisonTag: photo.comparisonTag, url: `/api/media/${photo.id}/thumbnail` }))}/>}
    {photos.length ? <BulkSelection action={bulkMoveToTrash.bind(null, "photo")} allIds={allPhotoIds.map((photo) => photo.id)} locale={locale}><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4" data-swipe-lock>{photos.map((photo) => <SelectableItem id={photo.id} key={photo.id}><article className="group min-w-0 overflow-hidden rounded-2xl border border-white/9 bg-[#0d141e]"><div className="relative aspect-[4/3] overflow-hidden bg-slate-900"><Image alt="" className="object-cover transition duration-500 group-hover:scale-[1.025]" fill sizes="(max-width:640px) 50vw, (max-width:1280px) 33vw, 25vw" src={`/api/media/${photo.id}/thumbnail`} unoptimized/>{photo.featuredAt && <span aria-label="Featured on website" className="absolute start-2 top-2 flex size-7 items-center justify-center rounded-full bg-[#102347]/90 text-blue-200" title="Featured on website"><Star fill="currentColor" size={13}/></span>}{photo.comparisonTag !== "UNTAGGED" && <span className="absolute bottom-2 start-2 rounded-full bg-black/75 px-2 py-1 text-[9px] font-bold tracking-wide text-white">{photo.comparisonTag}</span>}</div><div className="space-y-2 p-2.5"><div className="flex items-center justify-between gap-2"><span className="flex min-w-0 items-center gap-1.5 text-[10px] text-slate-600"><Images className="shrink-0" size={12}/><span className="truncate">{photo.width}×{photo.height}</span></span><div className="flex gap-1"><a aria-label="Download photo" className="icon-button size-8" download href={`/api/media/${photo.id}/download`} title="Download photo"><Download size={14}/></a>{canManage && <form action={setAppointmentPhotoFeatured.bind(null, photo.id, !photo.featuredAt)}><button aria-label={photo.featuredAt ? "Remove from website" : "Feature on website"} className="icon-button size-8" title={photo.featuredAt ? "Remove from website" : "Feature on website"}><Star fill={photo.featuredAt ? "currentColor" : "none"} size={14}/></button></form>}</div></div>{canManage && <div aria-label="Before or after tag" className="grid grid-cols-3 gap-1 rounded-lg bg-black/15 p-1"><form action={setAppointmentPhotoComparisonTag.bind(null, photo.id, "BEFORE")}><button aria-pressed={photo.comparisonTag === "BEFORE"} className={`w-full rounded-md px-1 py-1.5 text-[9px] font-semibold ${photo.comparisonTag === "BEFORE" ? "bg-blue-400/20 text-blue-200" : "text-slate-500 hover:text-slate-200"}`}>Before</button></form><form action={setAppointmentPhotoComparisonTag.bind(null, photo.id, "AFTER")}><button aria-pressed={photo.comparisonTag === "AFTER"} className={`w-full rounded-md px-1 py-1.5 text-[9px] font-semibold ${photo.comparisonTag === "AFTER" ? "bg-fuchsia-400/20 text-fuchsia-200" : "text-slate-500 hover:text-slate-200"}`}>After</button></form><form action={setAppointmentPhotoComparisonTag.bind(null, photo.id, "UNTAGGED")}><button aria-pressed={photo.comparisonTag === "UNTAGGED"} className={`w-full rounded-md px-1 py-1.5 text-[9px] font-semibold ${photo.comparisonTag === "UNTAGGED" ? "bg-white/8 text-slate-200" : "text-slate-500 hover:text-slate-200"}`}>None</button></form></div>}</div></article></SelectableItem>)}</div></BulkSelection> : <section className="empty">This album has no active photos.</section>}
