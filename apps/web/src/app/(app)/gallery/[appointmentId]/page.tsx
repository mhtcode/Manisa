...
import { ArrowLeft, CalendarDays, Images, Scissors, Star, Trash2, UserRound } from "lucide-react";
...
export default async function GalleryAlbumPage({ params, searchParams }: { params: Promise<{ appointmentId: string }>; searchParams: Promise<{ cursor?: string }> }) {
  const [{ appointmentId }, query, user] = await Promise.all([params, searchParams, requireUser()]);
  const cursor = decodeCursor(query.cursor);
  const appointment = await prisma.appointment.findFirst({ where: { id: appointmentId, deletedAt: null, status: "COMPLETED" }, include: { customer: true, actualServiceLines: { orderBy: { position: "asc" } }, photos: { where: { deletedAt: null, ...(cursor ? { OR: [{ createdAt: { lt: cursor.createdAt } }, { createdAt: cursor.createdAt, id: { lt: cursor.id } }] } : {}) }, take: PAGE_SIZE + 1, orderBy: [{ createdAt: "desc" }, { id: "desc" }] } } });
