...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<{ q?: string; from?: string }> }) {
  ...
  const [customers, deliveredAppointments, actualServiceLines, historicalServiceLines] = await Promise.all([
    prisma.customer.findMany({ where: { active: true, deletedAt: null }, include: { profilePhotos: { where: { deletedAt: null, status: "READY" }, select: { id: true }, orderBy: { createdAt: "desc" }, take: 1 }, _count: { select: { appointments: { where: { deletedAt: null } } } } }, orderBy: { updatedAt: "desc" } }),
