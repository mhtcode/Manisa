...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const now = new Date();
  const [customers, deliveredAppointments, serviceLines] = await Promise.all([
    ...
    prisma.appointmentService.findMany({ where: { appointment: { status: { in: ["COMPLETED", "HISTORICAL"] }, startAt: { lte: now } } }, select: { serviceNameSnapshot: true, appointment: { select: { customerId: true } } } }),
  ]);
  ...
  serviceLines.forEach((line) => { const counts = serviceCounts.get(line.appointment.customerId) || new Map<string, number>(); counts.set(line.serviceNameSnapshot, (counts.get(line.serviceNameSnapshot) || 0) + 1); serviceCounts.set(line.appointment.customerId, counts); });
