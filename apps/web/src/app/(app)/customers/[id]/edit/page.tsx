...
export default async function EditCustomerPage({ params }: { params: Promise<{ id: string }> }) { const { id } = await params; const user = await requireBusinessPermission("customers.manage"); const [customer, customers] = await Promise.all([prisma.customer.findUnique({ where: { id, businessId: user.businessId, deletedAt: null } }), prisma.customer.findMany({ where: { businessId: user.businessId, id: { not: id }, deletedAt: null, OR: [{ active: true }, { referrals: { some: { id } } }] }, orderBy: [{ firstName: "asc" }, { lastName: "asc" }] })]); if (!customer) notFound(); return <><PageHeading title="Edit customer" description="Update contact details, preferences, and referral source."/><CustomerForm customer={customer} action={updateCustomer.bind(null,id)} referralOptions={customers.map((item) => ({ id: item.id, name: customerName(item), phone: item.phone, email: item.email }))}/></>; }
