...
import { Banknote, CalendarCheck2, CalendarClock, Clock3, Contact, GitFork, Sparkles, Trash2, TrendingUp, UserCheck } from "lucide-react";
...
export default async function CustomerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const profilePhoto = await prisma.mediaAsset.findFirst({ where: { customerId: id, ownerType: "CUSTOMER_AVATAR", deletedAt: null, status: "READY" }, select: { id: true } });
  const now = new Date();
  ...
  return <>
    <PageHeading title={customerName(customer)} actions={<><Link className="button" href={`/appointments/new?customerId=${id}`}>+ Appointment</Link><Link className="button-secondary" href={`/customers/${id}/edit`}>Edit profile</Link></>}/>
