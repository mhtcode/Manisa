...
export default async function ServicesPage({ searchParams }: { searchParams: Promise<{ from?: string }> }) {
  ...
  const categories = await prisma.studioCategory.findMany({
    ...
    include: {
      services: {
        ...
        include: {
          actualAppointmentServices: { where: { appointment: { deletedAt: null, status: "COMPLETED" } }, select: { finalPrice: true, actualDurationMinutes: true } },
          appointments: { where: { deletedAt: null, status: "COMPLETED", actualServiceLines: { none: {} } }, select: { finalPrice: true, actualDurationMinutes: true } },
        },
  ...
  return <>
    <PageHeading backHref={query.from === "settings" ? "/settings" : undefined} title="Service catalog" description="Organize every studio category, service default, color choice, and actual performance." actions={<><ViewModeToggle initialMode={view} page="services"/><Link className="button" href="/services/new"><Plus size={17}/>New service</Link></>}/>
      ...
      {categories.map((category) => <CollapsibleCategory accentColor={category.accentColor} count={category.services.filter((service) => service.active).length} description={category.description || "Custom studio category"} icon={category.icon} initialCollapsed={sectionCollapsed(user.settings?.collapsedSections, `service-category-${category.id}`)} key={category.id} name={category.name} sectionId={`service-category-${category.id}`}>
        <div className={view === "grid" ? "grid gap-3 md:grid-cols-2 xl:grid-cols-3" : "space-y-3"}>
          {category.services.map((service) => {
            ...
            return <SelectableItem id={service.id} key={service.id}><article className={`panel p-5 ${service.active ? "" : "opacity-60"}`}>
              ...
              <div className="mt-4 flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] uppercase tracking-wider text-slate-600">Actual revenue · hourly</p><p className="mt-1 text-xs text-slate-300">{formatMoney(revenue, service.currency)} · {formatMoney(minutes ? revenue / (minutes / 60) : 0, service.currency)}</p></div><div className="flex flex-wrap gap-2"><Link className="button-secondary h-9 min-h-9 px-3" href={`/services/${service.id}/edit`}>Edit</Link><form action={toggleService.bind(null, service.id, !service.active)}><button className="button-secondary h-9 min-h-9 px-3">{service.active ? "Disable" : "Enable"}</button></form><ConfirmActionForm action={moveToTrash.bind(null, "service", service.id)} className="icon-button size-9 border-rose-400/20 text-rose-300" message={`Move ${service.name} to Trash? Appointment history will keep its saved service details. The service will be permanently deleted after seven days unless restored.`} title="Move service to Trash"><Trash2 size={14}/><span className="sr-only">Move service to Trash</span></ConfirmActionForm></div></div>
