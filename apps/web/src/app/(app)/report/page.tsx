...
export default async function ReportPage({ searchParams }: { searchParams: Promise<ReportQuery> }) {
  ...
  const data = await getReportData(query, timezone);
  ...
  return <>
    ...
    <div className="mt-4"><ReportCharts busiestDays={data.current.busiestDays} busiestHours={data.current.busiestHours} monthlyHours={data.current.monthlyHours} outcomes={data.current.outcomes} payments={data.current.payments} previousTrend={data.previous.timeline} queryString={chartParams.toString()} services={data.current.services} trend={data.current.timeline}/></div>

    <section className="panel mt-4 overflow-hidden" id="records"><div className="panel-header"><div><h2 className="font-semibold text-white">Filtered records</h2><p className="mt-1 text-xs text-slate-500">{data.recordCount} results · showing up to 250 newest</p></div><ViewModeToggle initialMode={view} page="reportRecords"/></div>{data.records.length ? <div className={view === "grid" ? "grid gap-3 p-3 sm:grid-cols-2 xl:grid-cols-3" : "divide-y divide-white/8"}>{data.records.map((appointment) => <Link className={view === "grid" ? "rounded-xl border border-white/8 bg-white/[0.02] p-4 hover:border-blue-400/20" : "flex items-center justify-between gap-3 px-4 py-3.5 hover:bg-blue-500/[0.04] sm:px-5"} href={`/appointments/${appointment.id}`} key={appointment.id}><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-100" dir="auto">{customerName(appointment.customer)}</p><p className="mt-1 truncate text-xs text-slate-500" dir="auto">{appointment.serviceNameSnapshot} · {formatBusinessDate(appointment.startAt, locale, timezone)}</p>{view === "grid" && <p className="mt-3 text-sm font-medium text-blue-200">{formatMoney(appointment.finalPrice || appointment.expectedPrice,appointment.currency,locale)}</p>}</div><div className="shrink-0 text-end"><StatusBadge status={appointment.status}/>{view === "list" && <p className="mt-1.5 text-xs text-slate-400">{formatMoney(appointment.finalPrice || appointment.expectedPrice,appointment.currency,locale)}</p>}</div></Link>)}</div> : <div className="empty">No records match this report.</div>}</section>
  </>;
}
