...
export async function POST(request: Request) {
  ...
  const { ownerType, customerId, appointmentId, fileName, contentType, sizeBytes } = parsed.data;
  ...
  return NextResponse.json({ assetId, uploadUrl: await createUploadUrl(key, contentType), expiresIn: 600 });
}
