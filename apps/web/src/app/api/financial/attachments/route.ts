...
export async function POST(request: Request) {
  ...
  const owner = ownerType === "bill" ? await prisma.supplierBill.findFirst({ where: { id: ownerId, businessId: user.businessId, deletedAt: null }, select: { id: true } }) : await prisma.financialTransaction.findFirst({ where: { id: ownerId, businessId: user.businessId, deletedAt: null }, select: { id: true } });
  if (!owner) return NextResponse.json({ error: "Financial record not found." }, { status: 404 });
  const business = await prisma.business.findUniqueOrThrow({ where: { id: user.businessId }, select: { storageQuotaBytes: true, storageUsedBytes: true } });
  if (business.storageUsedBytes + BigInt(file.size) > business.storageQuotaBytes) return NextResponse.json({ error: "The business storage quota is full." }, { status: 409 });
  const extension = mimeType === "application/pdf" ? "pdf" : mimeType.split("/")[1];
  const objectKey = `${user.businessId}/financial/${ownerType}/${ownerId}/${randomUUID()}.${extension}`;
  try {
    await putObject(objectKey, bytes, mimeType);
    const attachment = await prisma.$transaction(async (tx) => {
      const reserved = await tx.business.updateMany({ where: { id: user.businessId, storageUsedBytes: business.storageUsedBytes }, data: { storageUsedBytes: { increment: BigInt(file.size) } } });
      if (!reserved.count) throw new Error("Storage changed while uploading. Try again.");
      return tx.financialAttachment.create({ data: { businessId: user.businessId, billId: ownerType === "bill" ? ownerId : null, transactionId: ownerType === "transaction" ? ownerId : null, originalName: file.name.slice(0, 240), mimeType, objectKey, sizeBytes: file.size } });
    });
