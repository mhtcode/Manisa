...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const job = await prisma.exportJob.findFirst({ where: { id: (await params).id, status: "READY", expiresAt: { gt: new Date() } } });
  ...
  await prisma.auditLog.create({ data: { actorId: user.id, actorSnapshot: user.email, action: "data.export.download", targetType: "ExportJob", targetId: job.id } });
