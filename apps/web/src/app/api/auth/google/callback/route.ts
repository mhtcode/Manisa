...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const invitationPermissions = validInvitation?.permissionOverrides as Prisma.InputJsonValue | undefined;
    let user = await prisma.user.findFirst({ where: { OR: [{ googleSubject: profile.sub }, { email }] } });
    if (user) {
      if (!user.active || user.deletedAt) return loginRedirect(request, "inactive");
      if (user.googleSubject && user.googleSubject !== profile.sub) return loginRedirect(request, "email-linked");
      user = await prisma.user.update({ where: { id: user.id }, data: { googleSubject: profile.sub, name: profile.name?.trim() || user.name, ...(validInvitation ? { role: validInvitation.role, permissionOverrides: invitationPermissions } : {}) } });
      if (validInvitation) await prisma.invitation.update({ where: { id: validInvitation.id }, data: { acceptedAt: new Date(), acceptedById: user.id } });
    } else {
      if (!validInvitation) return loginRedirect(request, "signup-disabled");
      user = await prisma.$transaction(async (tx) => {
        const created = await tx.user.create({ data: { email, name: profile.name?.trim() || email.split("@")[0], googleSubject: profile.sub, passwordHash: null, role: validInvitation.role, permissionOverrides: invitationPermissions!, preferences: { create: {} } } });
