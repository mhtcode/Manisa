...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ google?: string; error?: string }> }) {
  ...
  return <main className="grid min-h-screen lg:grid-cols-[1.1fr_0.9fr]">
    ...
    <section className="flex items-center justify-center px-5 py-14 sm:px-10"><div className="w-full max-w-md"><div className="mb-10 lg:hidden"><BrandLink/></div><p className="text-sm font-medium text-teal-300">Welcome back</p><h2 className="mt-2 text-3xl font-semibold tracking-[-0.04em]">Sign in to Manisa</h2><p className="mt-3 text-sm leading-6 text-slate-500">Use your administrator account to continue.</p>{(accessError || (google && googleErrors[google])) && <p className="mt-5 rounded-xl border border-rose-400/20 bg-rose-400/8 px-4 py-3 text-sm text-rose-300">{accessError || googleErrors[google!]}</p>}<LoginForm/><div className="my-5 flex items-center gap-3 text-[11px] uppercase tracking-[.15em] text-slate-600"><span className="h-px flex-1 bg-white/8"/>or<span className="h-px flex-1 bg-white/8"/></div><GoogleAuthButton configured={googleAuthConfigured()}/></div></section>
  </main>;
}
