...
async function audit(user: Awaited<ReturnType<typeof requireBusinessPermission>>, action: string, targetType: string, targetId: string, after?: Record<string, unknown>) {
  await prisma.auditLog.create({ data: { actorId: user.id, actorSnapshot: user.email, action, targetType, targetId, after: after as Prisma.InputJsonValue | undefined } });
}
...
export async function createFinancialAccount(formData: FormData) {
  ...
  const position = await prisma.financialAccount.count({ where: { deletedAt: null } });
  const account = await prisma.financialAccount.create({ data: { name, type, openingBalance, position } });
...
export async function createFinancialCategory(formData: FormData) {
  ...
  const position = await prisma.financialCategory.count({ where: { type, deletedAt: null } });
  const category = await prisma.financialCategory.create({ data: { name, type, position } });
  await audit(user, "financial.category.create", "FinancialCategory", category.id, { name, type });
  refresh();
}

export async function updateFinancialCategory(id: string, formData: FormData) {
  const user = await requireBusinessPermission("financial.manage");
  const name = z.string().trim().min(1).max(80).parse(formData.get("name"));
  const category = await prisma.financialCategory.findFirst({ where: { id, deletedAt: null } });
  if (!category) throw new Error("Financial category not found.");
  await prisma.financialCategory.update({ where: { id }, data: { name } });
  await audit(user, "financial.category.update", "FinancialCategory", id, { before: category.name, name, type: category.type });
  refresh();
}
...
export async function createVendor(formData: FormData) {
  ...
  const vendor = await prisma.vendor.create({ data: { name, email: text(formData.get("email"), 160) || null, phone: text(formData.get("phone"), 50) || null } });
...
export async function createCashTransaction(formData: FormData) {
  ...
  const [account, destination, category, vendor] = await Promise.all([
    prisma.financialAccount.findFirst({ where: { id: accountId, active: true, deletedAt: null } }),
    destinationAccountId ? prisma.financialAccount.findFirst({ where: { id: destinationAccountId, active: true, deletedAt: null } }) : null,
    categoryId ? prisma.financialCategory.findFirst({ where: { id: categoryId, type: type === "INCOME" ? "INCOME" : "EXPENSE", active: true, deletedAt: null } }) : null,
    vendorId ? prisma.vendor.findFirst({ where: { id: vendorId, deletedAt: null } }) : null,
  ]);
  ...
  const transaction = await prisma.financialTransaction.create({ data: {
    type, accountId: account.id, destinationAccountId: destination?.id,
...
export async function createSupplierBill(formData: FormData) {
  ...
  const [vendor, category] = await Promise.all([
    prisma.vendor.findFirst({ where: { id: vendorId, active: true, deletedAt: null } }),
    prisma.financialCategory.findFirst({ where: { id: categoryId, type: "EXPENSE", active: true, deletedAt: null } }),
  ]);
  ...
  const bill = await prisma.supplierBill.create({ data: {
    vendorId: vendor.id, vendorNameSnapshot: vendor.name, billNumber: text(formData.get("billNumber"), 80) || null,
...
export async function paySupplierBill(billId: string, formData: FormData) {
  ...
  const [bill, account] = await Promise.all([
    prisma.supplierBill.findFirst({ where: { id: billId, deletedAt: null }, include: { lines: true, payments: { where: { deletedAt: null, archivedAt: null } } } }),
    prisma.financialAccount.findFirst({ where: { id: accountId, active: true, deletedAt: null } }),
  ]);
  ...
  const transaction = await prisma.financialTransaction.create({ data: {
    type: "BILL_PAYMENT", accountId: account.id, categoryId: category?.categoryId, vendorId: bill.vendorId, billId: bill.id,
...
export async function createRecurringBill(formData: FormData) {
  ...
  await prisma.recurringBillTemplate.create({ data: {
    vendorId: vendor.id, vendorNameSnapshot: vendor.name, categoryId: category.id, categoryNameSnapshot: category.name,
...
export async function createCustomerInvoice(appointmentId: string, formData: FormData) {
  const user = await requireBusinessPermission("financial.manage");
  const appointment = await prisma.appointment.findFirst({ where: { id: appointmentId, status: "COMPLETED", deletedAt: null }, include: { customer: true, actualServiceLines: true, invoices: true } });
  if (!appointment?.finalPrice) throw new Error("Only finalized appointments can be invoiced.");
  const dueInput = text(formData.get("dueDate"), 10);
  const invoice = await prisma.$transaction(async (tx) => {
    const settings = await tx.studioSettings.update({ where: { id: "studio" }, data: { nextInvoiceNumber: { increment: 1 } } });
    return tx.customerInvoice.create({ data: {
      appointmentId, invoiceNumber: settings.nextInvoiceNumber - 1, invoicePrefix: settings.invoicePrefix,
      revision: appointment.invoices.length + 1, businessSnapshot: { name: settings.name, address: settings.address, email: settings.publicEmail, phone: settings.publicPhone },
...
export async function updateFinancialRetention(formData: FormData) {
  ...
  if (user.role !== "OWNER") throw new Error("Only the owner can change financial retention.");
  const financialRetentionDays = z.coerce.number().int().min(7).max(3650).parse(formData.get("financialRetentionDays"));
  await prisma.studioSettings.update({ where: { id: "studio" }, data: { financialRetentionDays } });
  await audit(user, "financial.retention.update", "StudioSettings", "studio", { financialRetentionDays });
  refresh();
}
...
export async function restoreFinancialRecord(type: "transaction" | "bill" | "invoice" | "account" | "category" | "vendor", id: string) {
  ...
  await requireBusinessPermission("trash.manage");
  const model = type === "transaction" ? prisma.financialTransaction : type === "bill" ? prisma.supplierBill : type === "invoice" ? prisma.customerInvoice : type === "account" ? prisma.financialAccount : type === "category" ? prisma.financialCategory : prisma.vendor;
  const result = await (model as typeof prisma.financialTransaction).updateMany({ where: { id, deletedAt: { not: null }, archivedAt: null }, data: { deletedAt: null, purgeAt: null } });
...
export async function bulkMoveFinancialToTrash(type: "transaction" | "bill" | "invoice" | "account" | "category" | "vendor", formData: FormData) {
  ...
  const where = { id: { in: [...new Set(ids)] }, deletedAt: null };
