...
export async function createPaymentMethod(formData: FormData) {
  ...
  if (data.defaultAccountId && !(await prisma.financialAccount.count({ where: { id: data.defaultAccountId, deletedAt: null } }))) throw new Error("Financial account not found.");
...
export async function addAppointmentPayment(appointmentId: string, formData: FormData) {
  ...
  const method = await prisma.paymentMethod.findFirst({ where: { id: methodId, active: true, deletedAt: null }, include: { defaultAccount: true } });
  if (!appointment?.finalPrice || !method) throw new Error("Appointment or payment method not found.");
  const account = method.defaultAccount || await prisma.financialAccount.findFirst({ where: { name: "Undeposited funds", deletedAt: null } });
  const category = await prisma.financialCategory.findFirst({ where: { name: "Appointment services", type: "INCOME", deletedAt: null } });
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.appointmentPayment.create({ data: { appointmentId, paymentMethodId: method.id, methodNameSnapshot: method.name, amount, note, recordedById: user.id, transactionId: transaction.id } });
