...
export async function requestBusinessExport(formData: FormData) {
  ...
  const job = await prisma.$transaction(async (tx) => {
    const active = await tx.exportJob.count({ where: { status: { in: ["QUEUED", "PROCESSING"] } } });
    if (active) throw new Error("This business already has an export in progress.");
    return tx.exportJob.create({ data: { requestedById: user.id, fromDate, toDate } });
  }, { isolationLevel: "Serializable" });
  await prisma.auditLog.create({ data: { actorId: user.id, actorSnapshot: user.email, action: "data.export.create", targetType: "ExportJob", targetId: job.id, after: { fromDate, toDate } } });
  revalidatePath("/settings/data-transfer");
}

export async function retryBusinessExport(id: string) {
  const user = await requireBusinessPermission("data.export");
  await prisma.exportJob.updateMany({ where: { id, status: { in: ["FAILED", "EXPIRED"] } }, data: { status: "QUEUED", progress: 0, errorMessage: null, objectKey: null, expiresAt: null } });
  revalidatePath("/settings/data-transfer");
}

export async function deleteBusinessExport(id: string) {
  const user = await requireBusinessPermission("data.export");
  const job = await prisma.exportJob.findFirst({ where: { id, status: { not: "PROCESSING" } }, select: { objectKey: true } });
  if (!job) return;
