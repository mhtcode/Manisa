...
export const getCurrentUser = cache(async () => {
  ...
  const settings = membership ? {
    ...
    publicPhone: membership.business.settings?.publicPhone ?? null,
    publicEmail: membership.business.settings?.publicEmail ?? null,
    bookingUrl: membership.business.settings?.bookingUrl ?? null,
    studioTagline: membership.business.settings?.studioTagline ?? null,
    studioBiography: membership.business.settings?.studioBiography ?? null,
    financialRetentionDays: membership.business.settings?.financialRetentionDays ?? 2190,
    invoicePrefix: membership.business.settings?.invoicePrefix ?? "INV",
