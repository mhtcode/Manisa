...
describe("Google Calendar OAuth state", () => {
  it("round trips a signed user-scoped state", () => {
    const created = createGoogleCalendarOAuthState("user-1", secret, 1_000);
    expect(verifyGoogleCalendarOAuthState(created.state, created.nonce, secret, 2_000)).toMatchObject({ userId: "user-1" });
  });
