...
type StatePayload = { nonce: string; userId: string; expiresAt: number };

function sign(encoded: string, secret: string) {
  return createHmac("sha256", secret).update(encoded).digest("base64url");
}

export function createGoogleCalendarOAuthState(userId: string, secret: string, now = Date.now()) {
  const payload: StatePayload = { nonce: randomBytes(24).toString("base64url"), userId, expiresAt: now + 10 * 60_000 };
...
export function verifyGoogleCalendarOAuthState(state: string, nonce: string, secret: string, now = Date.now()) {
  try {
    ...
    if (payload.nonce !== nonce || !payload.userId || payload.expiresAt < now) return null;
    return payload;
  } catch { return null; }
}
