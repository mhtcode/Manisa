export type NotificationKind = "request" | "overdue" | "payment" | "confirm";
...
export function notificationAction(appointment: { id: string; status: string; paymentStatus: string; startAt: Date; expectedDurationMinutes: number }, now = new Date()) {
  if (appointment.status === "REQUESTED") return { key: `request:${appointment.id}`, kind: "request" as const, actionHref: `/appointments/${appointment.id}`, actionLabel: "Review request" };
