...
export function publicGalleryFullSource(source: string) {
  return source.startsWith("/public-media/gallery/") ? `${source}?view=full` : source;
}

export function publicProgressRailVisible(lastScrollAt: number | null, now: number, idleDelay = 900) {
  return lastScrollAt !== null && now - lastScrollAt < idleDelay;
}
...
export const publicCopy = {
  en: {
    ...
    ratingReviews: "reviews", studioKicker: "About us", studioTitle: "Slow beauty for busy lives.", studioBody: "Manisa is an appointment-only home studio for people who value relaxed care, precise technique, and results made personal.", appointmentOnly: "Appointment only",
  ...
  fa: {
    ...
    ratingReviews: "نظر", studioKicker: "درباره ما", studioTitle: "زیبایی آرام برای زندگی‌های پرمشغله.", studioBody: "مانیسا استودیویی خانگی و وقت‌محور برای کسانی است که به آرامش، ظرافت و نتیجه‌ای شخصی اهمیت می‌دهند.", appointmentOnly: "فقط با تعیین وقت",
